...
$year = $_SESSION['year'];
$lang = 'kz';
$sql = "select c_pril,name$lang from zayavlpril where type=2 order by name";
	selectdom($conapps,$sql,'c_pril','c_pril','Укажите отдел по делам обороны, получатель справки',0);

$sql = "select StudentID, concat(lastname,' ', left(firstname,1),'.', left(patronymic,1),'.'),ProfessionID,StudyLanguageID,TypCurriculumID,CourseNumber,studyformid, dopusk,groupID,specializationID,PaymentFormID,lastname,firstname,patronymic,DATE_FORMAT(BirthDate,'%d.%m.%Y'),TypCurriculumID,year(startdate),iinplt,YEAR(enroll_order_date) from students where StudentID='".$_SESSION['personid']."' and isstudent>0";
$res = $con->query($sql) or die($sql);
list($c_stud,$fio,$c_spec,$langid,$tup,$course,$c_form,$dopusk,$groupID,$specializationID,$pay,$s,$n,$p,$bd,$tid,$startdate,$iin,$y1)=mysqli_fetch_row($res);
$fiofull = "$s $n $p";
...
$data = str_replace('#YEARENTER#', $startdate, $data);
...
<script>
	...
	$('#c_pril').change(function() {
		...
		$('.sc').text($("#c_pril option:selected").text());
	}).change();
</script>