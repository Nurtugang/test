<?php
$sql = "select komid,status,date_format(dates,'%d.%m.%Y'),date_format(datee,'%d.%m.%Y'),header,finid,finpay,fin1,fin2,fin3,zpersonid from koms where komid=$komid";
$res = $condocs->query($sql);
list($komid,$status,$dates,$datee,$header,$finid,$finpay,$fin1,$fin2,$fin3,$zpersonid) = mysqli_fetch_row($res);
$sql = "select namekz from komfintypes where id='$finid'";
$res = $condocs->query($sql);
list($fin) = mysqli_fetch_row($res);
if ($finid == '016') {
    if ($fin1==1) {
        $fin .= '<br />тәуліктік шығындар';
    }
    if ($fin2==1) {
        $fin .= '<br />жол шығындары';
    }
    if ($fin3==1) {
        $fin .= '<br />жатын орын шығындары';
    }
}
...
$sign[3] = 0;
$sql = "select group_concat(dean) from structural_subdivision where id in (2,126,129)";
$res = $con->query($sql);
...
switch ($_SESSION['subdivision_type']) {
    ...
    case '3':
        ...
        $sql = "SELECT
            ...
            $res = $con->query($sql);
            list($dean) = mysqli_fetch_row($res);
            $sign[3] .= ','.$dean;
        break;
    default:
        //echo '<pre>';
...
$sql = "select tutorid from komfintypes where id='$finid'";
$res = $condocs->query($sql);
...
<div class="row">
    <div class="col-8">
        <div id="document">
            ...
            <b>Мені іссапарға жіберуіңзді сұраймын:</b>
            <table width="800">
                <tr>
                    <td width="200" valign="top"><b>Іссапар мақсаты:</b></td>
                    <td><?=$header?></td>
                </tr>
                <tr>
                    <td width="200" valign="top"><b>Іссапар мерзімі:</b></td>
                    <td><?=$dates?> - <?=$datee?></td>
                </tr>
                <tr>
                    <td width="200" valign="top"><b>Іссапар бағыты:</b></td>
                    <?php
                    $sql = "select routeid,country,city,uchr,date_format(rdates,'%d.%m.%Y'),date_format(rdatee,'%d.%m.%Y') from komroutes where komid=$komid";
                    $res= $condocs->query($sql);
                    if ($res->num_rows!=0) {
                        while (list($routeid,$country,$city,$uchr,$dates,$datee) = mysqli_fetch_row($res)) {
                        if ($country==113) {
                            $sql = "select namekz,full_namekz from center_kato where code='$city'";
                            $resc = $con->query($sql);
                            list($namekz,$full_namekz) = mysqli_fetch_row($resc);
                            $city = str_replace($city,'',$full_namekz);
                        }
                        ?>
                    <td><?=$city?> <small>(<?=$dates?> - <?=$datee?>)</small><br /><?=$uchr?><br /></td>
                        <?php
                        }
                    }
                    ?>
            ...
            ?>
            <br />
            Іссапар уақытында басшылық міндетін <b><?=fiokz($zpersonid,$con)?></b> жүктеуді сұраймын.
            <?php
            }
            ?>            
            <br />
            ...
            <table width="800">
                ...
                for ($i=0;$i<count($ss);$i++) {
                    $sql = "SELECT
                        distinct tutor_positions.NameKZ,
    ...
    <script type="text/javascript">
        ...
        function createCAdESFromBase64BackLogin(result) {
            if (result['code'] === "500") {
                ...
                data.append('doctypeid', "107");
                data.append('docname', 'Заявление на командировку <?=$_SESSION['lastname']?> <?=$_SESSION['firstname']?> <?=$_SESSION['patronymic']?>');
                ...
                data.append('filename','document107.html');
