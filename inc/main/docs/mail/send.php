    ...
    <div class="email-list ps ps--active-y">
      <table class="table-light" id="mytable">
        ...
        <tbody>
          ...
          $sql = "SELECT distinct emails.mailid,date_format(emails.maildate,'%d.%m.%Y %h:%i'),emails.mailtheme,concat(tutors.lastname,' ',left(tutors.firstname,1),'.',left(tutors.patronymic,1),'.')
          FROM nitroapps.emailsto INNER JOIN nitroapps.emails ON emailsto.mailid = emails.mailid INNER JOIN nitro.tutors ON tutors.TutorID = emails.mailfrom WHERE emails.mailstatus = 1
          AND emails.mailfrom = $personid";
