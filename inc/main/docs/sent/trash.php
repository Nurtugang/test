<?php
if (isset($undelete)) {
	$sql = "update documents set status=1 where documentid=$documentid";
	$condocs->query($sql);
}
$type = 6;
?>
...
<script>
  ...
  function untrash(documentid) {
    //var w = $(window).width()-220;
    $('#main').load('modules.php?pa=docs-trash-sent&undelete=undelete&documentid='+documentid)
  };
